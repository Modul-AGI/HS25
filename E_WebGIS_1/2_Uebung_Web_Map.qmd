...
3. Gebe der Karte einen Namen (Zecken Stadt Zürich Vorname Name), schreibe eine kurze Zusammen-fassung und definiere Tags. Wähle folgende Einstellungen:
    ...
    - Share With: 	Groups (wähle «Modul Angewandte Geoinformatik HS 2025»; die Karte und die Feature Layers werden nur für diese Gruppe freigegeben). 
...
2. Gebe dem Layer den Namen «Velowege Vorname Name», schreibe eine kurze Zusammenfassung und definiere Tags. Wähle folgende Einstellungen:
    ...
    - Share With: 	Groups (wähle «Modul Angewandte Geoinformatik HS 2025»; der Feature Layer wird nur für diese Gruppe freigegeben).
...
4. Ergänze die Web App nun noch mit einem zusätzlichen WMS-Layer. Gehe hierfür zuerst auf www.geolion.zh.ch und suche nach «Klimamodell». Gehe auf «Klimamodell 2024: Analysekarten (Raster)», öffne das Register «Datenbezug» und wähle den Link unter Datenbezug -> «WMS». Wir brauchen den GetCapabilities Link.
...
5. Wechsle in ArcGIS Online auf «Inhalt» und füge den WMS-Klimamodell via « + Neues Element» hinzu. Wähle die Option «URL».
    ...
    Wähle in der Liste den WMS-Layer «Lufttemperatur Nacht [C°], 4 Uhr».

    Definiere folgende Parameter:
    -	Titel = Lufttemperatur Nacht
    -	Ordner = Modul AGI
    -	Tags = Temperatur
    -	Zusammenfassung = Lufttemperatur Nacht auf 2m Höhe über Grund um 4 Uhr

    ![](figures/arcgis_online_44.PNG)

6. Öffne in ArcGIS Online die Web Map «Zecken Stadt Zürich Vorname Name». Gehe auf «Hinzufügen > Layer durchsuchen» und füge den Layer «Lufttemperatur Nacht» zur Web Map hinzu. Verändere die Layerhierarchie und platziere die Temperaturkarte zuunterst.

7. Speichere die Änderungen.

8. Öffne nun die Web App. Der Layer «Lufttemperatur Nacht» ist nun auch in der Web App enthalten und kann je nach Bedarf ein- und ausgeblendet werden (so wie alle anderen Layer auch). 
