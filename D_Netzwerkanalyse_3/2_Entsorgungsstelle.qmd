...
Um aus den Linien eine Oberfläche zu erhalten müssen wir die Kategorien aus dem Output von `v.net.iso` interpolieren. Dies funktioniert besser mit Punkten als Linien. In einem ersten Schritt müssen wir also den Output aus `v.net.iso` in Punkte umwandeln. Verwenden Sie hierfür das Werkzeug *Extract Node* (Vector → Geometry Tools → Extract Node)
<!-- "Convert Line/Polygon Vertices to Points" (SAGA). Belassen sie das Häkchen bei "Insert additional Points" und setzten Sie die Distanz ("Insertion Distance") auf 100 Meter. Führen Sie das Werkzeug mit "run" aus. -->
...
Nun haben wir Punkte, die mit einer Interpolation in eine Oberfläche überführt werden können. Verwenden Sie dazu das Tool *IDW interpolation*. Überlegen sie, was Sie interpolieren möchten und legen sie das entsprechende Feld in der Option "Attribute" fest. 

<!-- Zusätzlich können Sie folgende Parameter wählen: -->
